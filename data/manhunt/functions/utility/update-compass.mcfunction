execute as @a[team=hunters] store result score @s has-compass run clear @s minecraft:compass 0
kill @e[type=item,nbt={Item: {id: "minecraft:compass"}}]
give @a[team=hunters, scores={has-compass=0}] minecraft:compass{display: {Name: '{"text": "Hunter Compass", "italic": false}'}, LodestoneDimension: "minecraft:overworld", LodestoneTracked: 0b}
